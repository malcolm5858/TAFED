...
def get_distance(station, latitude, longitude):
    return math.sqrt((latitude - station.latitude) ** 2 + (longitude - station.longitude))

def get_closest_station(latitude, longitude):
    closest = stations[0]
    for station in stations:
        if get_distance(station, latitude, longitude) < get_distance(closest, latitude, longitude):
            closest = station
    return closest
